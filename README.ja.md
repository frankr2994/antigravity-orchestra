...
| PowerShell 5.1+ | `$PSVersionTable.PSVersion` | Windows 標準搭載。PowerShell 7（`pwsh`）でも可 |
...
> ⚠️ **オーケストラ機能は対話モード（TUI）で使ってください。** ヘッドレスモード（`agy -p "..."`）ではワークスペースの customization（`.agents/` スキルと `AGENTS.md`）が注入されないため（agy 1.1.5 で実測確認）、`/startproject` 等のワークフローは動きません。ヘッドレスは単発の質問専用です。
...
スラッシュコマンドはルートの `AGENTS.md` が `.agents/workflows/*.md` へ橋渡しすることで動きます。`/startproject` がただの文字列として扱われる場合は、`AGENTS.md` がワークスペース直下にあるか、プロジェクトフォルダ内で `agy` を起動したかを確認してください。
...
```
my-project/
├── AGENTS.md             # agy が最初に読む中核指示（スラッシュコマンド対応表）
├── .agents/
│   ├── workflows/        # 6 ワークフロー
...
├── docs/                 # 知識ベース
│   ├── DESIGN.md             # 設計決定記録
│   ├── walkthrough.ja.md     # 実行ウォークスルー（英日）
│   ├── research/             # リサーチ結果
│   └── libraries/            # ライブラリ制約
│
└── logs/
...
**トリガーキーワード**（代表例。全量は正本 [`.agents/skills/codex-system/SKILL.md`](.agents/skills/codex-system/SKILL.md) を参照）：

| 分類 | 代表例 |
|------|--------|
| 設計系 | 「設計」「アーキテクチャ」「トレードオフ」 |
| デバッグ系 | 「なぜ動かない」「エラー」「バグ」 |
| レビュー系 | 「レビュー」「チェック」 |
...
Plus（$20/月）で十分使えます。Pro（$200/月）はより多くの使用量が必要な場合に検討してください。Codex CLI は API キー認証にも対応しています。
...
| 問題 | 解決策 |
|------|--------|
| `running scripts is disabled on this system` | `Set-ExecutionPolicy -Scope CurrentUser RemoteSigned` を実行して再試行 |
| `/startproject` がただの文字列として扱われる | `AGENTS.md` がワークスペース直下にあるか、プロジェクトフォルダ内で `agy` を対話モード（`-p` でなく）起動したか確認 |
