...
export function TaskActivity({ task, events, models }: { task: Task; events: TaskEvent[]; models: Record<string, string> | null }) {
  const recent = events.filter((event) => ['agent.started', 'agent.output', 'agent.completed', 'task.paused', 'task.resumed', 'task.provider-recovery', 'task.model-takeover', 'task.takeover_local', 'task.no-changes', 'task.review-cap', 'mcp.capability', 'mcp.tool', 'verification.result', 'git.commit', 'git.push', 'cloud.activity', 'cloud.completed', 'cloud.reviewing', 'cloud.reviewed', 'cloud.repair_requested', 'cloud.cancelled', 'cloud.integrated', 'warning'].includes(event.type)).slice(-8);
