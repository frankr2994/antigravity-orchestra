...
export function codexShellGuidance(platform = process.platform) {
  return platform === 'win32'
    ? '\nThis host is Windows. Use PowerShell-compatible commands, one command per tool call. Quote ripgrep alternation patterns with double quotes, do not use cmd/findstr fallbacks, and prefer Rider semantic reads when shell quoting would be fragile. Ripwire is available on PATH as `ripwire`: prefer `ripwire . --for="<task>"`, `ripwire . --callers=SYM`, `ripwire . --impact=SYM`, and `ripwire . --expand=SYM --top-k=0` over reading whole files into context. Use the exact selector Ripwire prints: prefer `path/to/file:Symbol` when a name is ambiguous, and use a bare symbol only when it is unambiguous. Keep `--top-k=0` for targeted `--expand` payloads so the ranked map is not repeated. Always pass `.` as the target directory from the project root.'
    : '\nRipwire is available on PATH as `ripwire`: prefer `ripwire . --for="<task>"`, `--callers=SYM`, `--impact=SYM`, and `--expand=SYM --top-k=0` over reading whole files into context. Use the exact selector Ripwire prints: prefer `path/to/file:Symbol` when a name is ambiguous, and use a bare symbol only when it is unambiguous. Keep `--top-k=0` for targeted `--expand` payloads so the ranked map is not repeated.';
}
