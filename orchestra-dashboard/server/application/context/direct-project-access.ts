...
export function directProjectAccessInstruction(rootInput: string, provider: 'gemma' | 'codex' | 'antigravity', riderAvailable = false) {
  ...
  const ripwireNote = isRipwireAvailable() ? ' and Ripwire on PATH (`ripwire`) for sub-second codebase maps, caller analysis, and quality deltas' : '';
  if (provider === 'gemma') {
    return `Authoritative project root: ${root}\nOrchestra supplies safe read-only project tools for this exact root${riderNote}${ripwireNote}. Use them when the answer depends on files or code structure. Never claim the project or tools are unavailable merely because arbitrary Bash is disabled.`;
  }
  return `Authoritative project root: ${root}\nThis turn has read-only filesystem access rooted at that project${riderNote}${ripwireNote}. Inspect only what directly answers the question. Do not run builds, tests, type checks, linters, or broad diagnostics unless the user explicitly asks for them. Never claim the project or tools are unavailable unless an actual read operation fails; if one fails, report the exact boundary.`;
}
...
export function isRipwireAccessQuestion(prompt: string): boolean {
  const text = prompt.trim();
  if (!text) return false;
  return /\bripwire\b/i.test(text);
}
...
export function deterministicDirectProjectAnswer(rootInput: string, prompt: string, agent: AgentName, riderAvailable = false): DirectProjectAnswer | null {
  const root = requireReadableProjectRoot(rootInput);
  const hasRider = isRiderAccessQuestion(prompt);
  const hasRipwire = isRipwireAccessQuestion(prompt);
  const hasProject = isProjectAccessQuestion(prompt);

  if (hasProject || hasRider || hasRipwire) {
    const isToolOnly = (hasRider || hasRipwire) && !/\b(?:all\s+files|all\s+of\s+its\s+files|directory|filesystem)\b/i.test(prompt);
    if (isToolOnly) {
      const ripwireAvailable = isRipwireAvailable();
      const parts: string[] = [];

      if (hasRipwire && hasRider) {
        if (riderAvailable && ripwireAvailable) {
          return {
            phase: 'direct-project-access',
            answer: `Yes. Both JetBrains Rider MCP and Ripwire are connected and active for this project. Rider supplies solution-aware semantic tools (\`rider_get_solution_projects\`, \`rider_search_symbol\`), and Ripwire supplies sub-second codebase maps (\`--for\`), blast-radius analysis (\`--situ\`), quality regressions (\`--quality-delta\`), and test gates (\`--test-gate\`).`,
          };
        }
      }
      ...
      if (hasRipwire) {
        if (ripwireAvailable) {
          parts.push(`Ripwire is installed and active for this project. Orchestra automatically uses Ripwire for task-oriented symbol maps (\`--for\`), situational blast-radius awareness (\`--situ\`), code-quality regressions (\`--quality-delta\`), and minimal test-gate calculation (\`--test-gate\`), and it is available on PATH as \`ripwire\`.`);
        } else {
          parts.push(`Ripwire is currently not detected at the configured path.`);
        }
      }

      if (hasRider) {
        if (riderAvailable) {
          parts.push(`JetBrains Rider MCP is connected and active for this project with read-only inspection tools (including \`rider_get_solution_projects\`, \`rider_get_file_problems\`, \`rider_search_in_files_by_text\`, and \`rider_search_symbol\`).`);
        } else {
          parts.push(`JetBrains Rider MCP is currently not connected or available for this project.`);
        }
      }

      const affirmative = (hasRipwire ? ripwireAvailable : true) && (hasRider ? riderAvailable : true);
      return {
        phase: 'direct-project-access',
        answer: `${affirmative ? 'Yes.' : 'No.'} ${parts.join(' ')}`,
      };
    }
