...
import { GemmaMicroEditService } from '../gemma/micro-edit-service.js';
...
export { evaluateRunHealth } from './run-monitor-service.js';
...
export class TaskExecutionCoordinator {
  ...
  private readonly directTasks: DirectTaskExecutor;
  private readonly conversationContext: TaskConversationContext;
  ...
  constructor(private readonly store: Store, maxGlobal = 2) {
    ...
    this.directTasks = new DirectTaskExecutor(store, {
      ...
      emit: (taskId, agent, type, payload) => this.emit(taskId, agent, type, payload),
  ...
  private async execute(taskId: string, signal: AbortSignal) {
    ...
    try {
      ...
      if (evidence && shouldAttemptGemmaAnswer(classification, task.prompt)) {
        this.transition(taskId, 'running');
        this.emit(taskId, 'gemma', 'agent.started', { phase: 'repository-answer', model: activeGemmaModel, evidenceFiles: evidence.includedFiles.length, estimatedTokens: evidence.estimatedTokens });
        try {
      ...
      if (!classification.mutating && evidence) agentResult.text = await this.conversationContext.validate(taskId, project.root, task.prompt, agentResult.text, evidence);
