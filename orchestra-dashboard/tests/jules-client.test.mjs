...
test('Phase 6 Jules Client — approvePlan, sendMessage, deleteSession, and activities endpoints', async () => {
  ...
  // 5. Activity polling uses only provider-documented pagination fields.
  await client.listActivities('12345', 100, 'next-page');
  assert.match(calls[4].url, /pageSize=100/);
  assert.match(calls[4].url, /pageToken=next-page/);
  assert.doesNotMatch(calls[4].url, /createTime=/);
});
